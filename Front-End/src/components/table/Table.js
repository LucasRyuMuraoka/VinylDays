...
const Table = ({ filter }) => {
	...
	const handleDeleteAlbum = () => {
		...
		albumsService.delete(selectedAlbum.id).then(async () => {
			toast.update(alert, {
				render: "Deleted! Await auto redirect...",
				type: "warning",
				isLoading: false,
				position: "top-right",
				autoClose: 2000,
	...
	const handleOpenDeleteAlbumModal = (album) => {
  ...
  return (
		<>
			<Container>
									...
									albumsWithFilter.map(album =>
										<tr key={ album.id }>
											<td className="text-center align-middle">
												<Image src={ album.URL } alt="Album Art"/>
											</td>
											<td>
												<AlignCenter>
													<TableData>
											...
											<td className="text-center align-middle">
												<BtnAction onClick={ () => handleOpenDeleteAlbumModal(album) } width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
				...
				{
					(deleteAlbumModalIsOpen) && <DeleteAlbumModal
																				isOpenned={ deleteAlbumModalIsOpen }
